...
class SearchProductViewModel @Inject constructor(
        ...
        firestore.collection("Products").get()
            .addOnSuccessListener { result ->
                ...
                viewModelScope.launch {
                    ...
                    _filteredProducts.value = allProductsList
                }
    ...
    fun filterProducts(query: String) {
        val currentProducts = (_allProducts.value as? Resource.Success)?.data ?: emptyList()
        _filteredProducts.value = if (query.isEmpty()) {
            currentProducts
        } else {
            currentProducts.filter { it.name.contains(query, ignoreCase = true) }
        }
    }
